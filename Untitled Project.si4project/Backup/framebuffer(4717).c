...
{
	const unsigned char *pData = pBMP->pData;
	int y, x;
	unsigned int cnt, a = 0;;
	if((pBMP->bpp != 32) && (pBMP->bpp != 24)) {
		fprintf(stderr, "BPP %d is not support.\n", pBMP->bpp);
			return;
	}

	for (y = pBMP->height + y0; y >= y0; y--)
	{
		for (x = x0; x < pBMP->width + x0; x++){
			cnt = WIDTH * y + x;		//当前像素点在屏幕的位置		
			*(pfb + cnt) = ((pData[a+0]<<16) | (pData[a+1]<<8) | (pData[a+2]<<0));
			a += 3;
		}
...
void fb_draw_JPEG_picture(int x0, int y0, struct picture_info *pJPEG)
{
	const unsigned char *pData = pJPEG->pData;
	int y, x;
	unsigned int cnt, a = 0;;
	if((pJPEG->bpp != 32) && (pJPEG->bpp != 24)) {
		fprintf(stderr, "BPP %d is not support.\n", pJPEG->bpp);
			return;
	}

	for (y = y0; y < pJPEG->height + y0;  y++)
	{
		for (x = x0; x < pJPEG->width + x0; x++){
			cnt = WIDTH * y + x;		//当前像素点在屏幕的位置		
			*(pfb + cnt) = ((pData[a+0]<<16) | (pData[a+1]<<8) | (pData[a+2]<<0));
			a += 3;
		}
	}
	debug("JPEG DEBUG ( %d	,  %d )\n", x, y);
}
